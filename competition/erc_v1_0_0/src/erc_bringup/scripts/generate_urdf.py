...
def generate_urdf(**xacro_args):
    ...
    for segment in ('inner_finger_left', 'inner_finger_right',
                    'outer_finger_left', 'outer_finger_right',
                    'fingertip_left', 'fingertip_right'):
        marker = f'<gazebo reference="gripper_left_{segment}_link">'
    ...
    for segment in ('inner_finger_left', 'inner_finger_right',
                    'outer_finger_left', 'outer_finger_right',
                    'fingertip_left', 'fingertip_right'):
        marker = f'<joint name="gripper_left_{segment}_joint"'
        start = urdf.index(marker)
        end = urdf.index('</joint>', start)
        block = urdf[start:end]
        block = block.replace('effort="0.1"', 'effort="1.0"')
        urdf = urdf[:start] + block + urdf[end:]
