...
def generate_launch_description():
    ...
    # The image ships only the KDL plugin, which solves a 7 DoF arm by random
    # restarts from the current state until its timeout expires. At .15 s it
    # regularly returns NO_IK_SOLUTION for poses that are plainly reachable,
    # which surfaces as OMPL failing to sample any goal state and aborting the
    # motion. Measured on the shipped model: at a low shelf row every stand-off
    # except the one nearest the seed state failed, with and without collision
    # checking. A longer budget is the only lever available here.
    kinematics={group:{'kinematics_solver':'kdl_kinematics_plugin/KDLKinematicsPlugin',
                       'kinematics_solver_search_resolution':.005,
                       'kinematics_solver_timeout':.6,
                       'kinematics_solver_attempts':5}
                for group in ('left_arm','left_arm_torso','right_arm')}
    limits={}
    for joint in robot.findall('joint'):
